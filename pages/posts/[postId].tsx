...
export default function Post() {
    ...
    const post: PostInterface = useSelector((s) =>
        s.posts.data.find((post) => String(post.id) === String(router.query.postId)),
    );
                    ...
                    <PostComments>
                        ...
                        {post.comments?.length > 0 ? (
                            post.comments.map((comment) => (
                                <PostSingleComment key={comment.id}>{comment.body}</PostSingleComment>
                            ))
                        ) : (
                            <div>No comments</div>
                        )}
